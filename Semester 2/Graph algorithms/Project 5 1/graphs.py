...
def bronKerbosch(graph, clique, vertices, excluded, cliques):
    # if vertices and excluded vertices are empty, it returns the clique as maximal clique
    if not vertices and not excluded:
        if len(clique) >= 2:
            cliques.append(clique)
        return

    # for every vertex in vertices of graph
    for v in list(vertices):
        # applies intersection between neighbors to see which match which
        new_vertices = vertices.intersection(graph.get_neighbors(v))
        new_excluded = excluded.intersection(graph.get_neighbors(v))

        # then recall this algorithm inside itself to compute the maximal clique
        bronKerbosch(graph, clique + [v], new_vertices, new_excluded, cliques)

        # after it will return the maximal clique, it will remove vertex from the vertices set and add it to the excluded set
        vertices.remove(v)
        excluded.add(v)
