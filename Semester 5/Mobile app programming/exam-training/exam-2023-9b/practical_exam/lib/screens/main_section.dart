...
class _MainSectionState extends State<MainSection> {
  ...
  Future<void> deleteEntity(int entityId) async {
    try {
      await ApiService.instance.deleteEntity(entityId);

      setState(() {
        entities.removeWhere((entity) => entity.id == entityId);
      });
    } catch (e) {
      logger.log(Level.info, "Error deleting entity");
    }
  }
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: IconButton(
                            ...
                            onPressed: () {
                              if (widget.online) {
                                deleteEntity(entities[index].id!);
                              } else {
                                popUpDialog(context, "Error",
                                    "Operation not available offline");
                              }
                            }),
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => FutureBuilder<Entity>(
                                future: api.getPet(entities[index].id!),
                                builder: (context, snapshot) {
                                  if (snapshot.connectionState ==
                                    ...
                                    return DetailsView(entity: snapshot.data!);
                                  }
                                },
                              ),
                            ),
