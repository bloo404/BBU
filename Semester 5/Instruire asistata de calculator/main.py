...
def process_image(img_rgb):
    ...
    if lines is not None:
        ...
        if closest_lines is not None:
            ...
            print(f"Distance between closest lines: {distance_cm:.2f} cm")
...
def to_display(img, label_widget):
    image = Image.fromarray(img)
    image = image.resize((400, 400))
    pic = ImageTk.PhotoImage(image)
    label_widget.configure(image=pic)
    label_widget.image = pic


def switch():
    global count
    count += 1
    if count > len(images_list) - 1:
        count = 0

    img_rgb, img_clustered = process_image(images_list[count][0])

    to_display(img_rgb, label_original)

    to_display(img_clustered, label_contours)

if __name__ == "__main__":
    path = "ivc/"
    images_list = load_images(path)

    count = 0
    win = Tk()
    win.title("Image changer")
    win.geometry("1200x600")

    label_original = Label(win, bg="black")
    label_original.place(x=100, y=50, width=400, height=400)

    label_contours = Label(win, bg="black")
    label_contours.place(x=700, y=50, width=400, height=400)

    right = Button(win, text="Change image", bg="gray", fg="white", command=switch)
    right.place(x=525, y=500, width=150)
