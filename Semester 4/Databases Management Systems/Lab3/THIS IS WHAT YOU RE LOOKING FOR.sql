...
CREATE OR ALTER PROC dropTables
AS
	drop table drink
	drop table bar
	drop table ticket
...
AS
	...
	-- Actor
	create table actor(AID int IDENTITY(1,1) primary key, actor_dob date)

	-- Theatrical piece
	create table theatrical_piece(TPID int IDENTITY(1,1) primary key, theatrical_piece_title varchar(100), theatrical_piece_author_last_name varchar(50),
	theatrical_piece_author_first_name varchar(50))

	-- Play
	create table play(PID int IDENTITY(1,1) primary key, TPID int foreign key references theatrical_piece(TPID), AID int foreign key references actor(AID),
	play_date date)
...
CREATE OR ALTER PROC insertInAll
	AS
	...
	insert into dbo.actor (actor_dob) values ('2000-06-30')
	insert into dbo.actor (actor_dob) values ('1999-06-30')
	insert into dbo.actor (actor_dob) values ('1998-06-30')
	insert into dbo.actor (actor_dob) values ('2000-05-29')
	insert into dbo.actor (actor_dob) values ('2000-03-16')
	insert into dbo.actor (actor_dob) values ('2001-06-30')
	insert into dbo.actor (actor_dob) values ('2002-06-30')
	insert into dbo.actor (actor_dob) values ('1997-11-09')
	insert into dbo.actor (actor_dob) values ('1989-12-25')
	insert into dbo.actor (actor_dob) values ('1986-11-25')
	...
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'Romeo & Juliette', 'Shakespeare', 'William')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'The Government Inspectior', 'Gogol', 'Nikolai')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'Old Times', 'Pinter', 'Harold')
	insert into dbo.theatrical_piece (theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'Purgatory in Ingolstadt', 'Fleisher', 'Marieluise')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'The Changeling', 'Middleton - Rowley', 'Thomas - William')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'Henry IV', 'Pirandello', 'Luigi')
	insert into dbo.theatrical_piece (theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'A Raisin in the Sun', 'Hansberry', 'Lorraine')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'An Oak Tree', 'Crouch', 'Tim')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'Sizwe Bansi is Dead', 'Fugard', 'Athol')
	insert into dbo.theatrical_piece ( theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
	values ( 'A streetcar Named Desire', 'Williams', 'Tennessee')
	...
	insert into dbo.see(seed,cid,tpid,see_date) values(0,1,1,'2004-05-26')
	...
	insert into dbo.play( tpid, aid, play_date) values (1,1,'2016-10-18')
	insert into dbo.play(tpid, aid, play_date) values (3,1,'2003-03-18')
	insert into dbo.play( tpid, aid, play_date) values (2,2,'2003-03-26')
	insert into dbo.play( tpid, aid, play_date) values (4,2,'2003-07-19')
	insert into dbo.play( tpid, aid, play_date) values (5,5,'2018-08-24')
	insert into dbo.play( tpid, aid, play_date) values (6,6,'2020-01-23')
	insert into dbo.play( tpid, aid, play_date) values (7,8,'2015-06-27')
	insert into dbo.play( tpid, aid, play_date) values (8,9,'2014-05-30')
	insert into dbo.play( tpid, aid, play_date) values (9,4,'2022-04-05')
GO
...
--											  2023
GO
create or alter procedure logTable
AS
DROP TABLE log
CREATE TABLE log (
...
-- remarks: req 1 & 2 + validate with errors + generalization (parametri) + use id from the table (not hardcoded/parameter)
-- remarks: 2 files for every special case (dirty read...) (done)
...
CREATE OR ALTER PROCEDURE deleteInsertedData
    ...
    @aid INT
AS
	DELETE FROM dbo.play WHERE pid = @pid AND tpid = @tpid AND aid = @aid
	DELETE FROM dbo.actor WHERE AID =  @aid
	DELETE FROM dbo.theatrical_piece WHERE TPID = @tpid
GO

-- Validators

CREATE FUNCTION ValidateString( @string VARCHAR(50))
RETURNS BIT
AS
BEGIN
    DECLARE @valid BIT = 1
...
CREATE OR ALTER FUNCTION ValidateDate(@date DATE)
RETURNS BIT
AS
BEGIN
	DECLARE @valid BIT = 1
...
GO
    ...
    -- rest of the stored procedure code
...
BEGIN
    BEGIN TRY
        ...
        INSERT INTO theatrical_piece values ( 'New piece', 'Willy', 'Wonka')
        ...
        INSERT INTO actor values( '1999-03-18')
        IF @@ROWCOUNT < 1
        BEGIN
            PRINT 'Problem with inserting values'
            ROLLBACK TRAN
        ...
        INSERT INTO play values(12, 11,'2023-03-18')
        IF @@ROWCOUNT < 1
        BEGIN
            PRINT 'Problem with inserting values'
            ROLLBACK TRAN
...
BEGIN TRAN
    BEGIN TRY
		...
		DECLARE @validLast BIT = dbo.ValidateString(@theatrical_piece_author_last_name)
		DECLARE @validTitle BIT = dbo.ValidateString( @theatrical_piece_title)
		DECLARE @validFirst BIT = dbo.ValidateString(@theatrical_piece_author_first_name)
		DECLARE @validPlayDate BIT = dbo.ValidateDate(@play_date)
		DECLARE @validActorDob BIT = dbo.ValidateDate(@actor_dob)

		IF @validLast = 0 OR @validTitle = 0 OR @validFirst = 0 OR @validPlayDate = 0 OR @validActorDob = 0
		BEGIN
        ...
        INSERT INTO dbo.theatrical_piece (theatrical_piece_title, theatrical_piece_author_last_name, theatrical_piece_author_first_name)
        VALUES (@theatrical_piece_title, @theatrical_piece_author_last_name, @theatrical_piece_author_first_name)
        IF @@ROWCOUNT < 1
        BEGIN
            PRINT 'THEATRICAL PIECE: Something went wrong with the insert.'
            ROLLBACK TRAN
            RETURN
        END
		
        INSERT INTO dbo.actor (actor_dob) VALUES (@actor_dob)
        IF @@ROWCOUNT < 1
        BEGIN
            PRINT 'ACTOR: Something went wrong with the insert.'
            ROLLBACK TRAN
            RETURN
        END

        INSERT INTO dbo.play (tpid, aid, play_date) VALUES (SCOPE_IDENTITY(), SCOPE_IDENTITY(), @play_date)
        IF @@ROWCOUNT < 1
        BEGIN
            PRINT 'PLAY: Something went wrong with the insert.'
            ROLLBACK TRAN
            RETURN
        END
        ...
        VALUES ('insertDataRequirement2', 
                CONCAT('theatrical_piece_title=', @theatrical_piece_title, 
                       ', theatrical_piece_author_last_name=', @theatrical_piece_author_last_name, 
                       ', theatrical_piece_author_first_name=', @theatrical_piece_author_first_name, 
                       ', actor_dob=', CONVERT(VARCHAR(10), @actor_dob, 120), 
                       ', play_date=', CONVERT(VARCHAR(10), @play_date, 120)), 
                'Insert successful')
...
EXEC insertDataRequirement2
    @theatrical_piece_title = 'Some new theatrical piece',
    @theatrical_piece_author_last_name = 'Donald',
    @theatrical_piece_author_first_name = 'Dux',
    @actor_dob = '1999-03-18',
